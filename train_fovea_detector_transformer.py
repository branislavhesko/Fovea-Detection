...
from models.cross_focal_net import FocalNet
...
class Trainer:
    ...
    def __init__(self, config: Config):
        ...
        self._fine_loss = torch.nn.MSELoss()
    ...
    def _train_single_epoch(self, epoch):
        ...
        for idx, data in enumerate(t):
            ...
            gt_points = torch.stack(torch.where(labels == 1))
            coarse, fine = self._model(img)
            loss = self._loss(F.interpolate(labels, scale_factor=1. / 16.), coarse)
